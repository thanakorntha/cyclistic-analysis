...
In order to facilitate ease of reference, Table 2 presents an overview of the descriptive statistics for the ride duration of Cyclistic bikes, encompassing all users including casual riders and annual members. 
...
trip_data_v2 %>% 
  ...
  knitr::kable(
    format = "simple", 
    caption = "An overview of the descriptive statistics for the ride duration in the Cyclistic's historical trip data in 2022"
  )
```

In total, there were `r prettyNum(count(trip_data_v2), big.mark = ",")` Cyclistic rides recorded in 2022. When discussing the duration of the rides, the minimum recorded duration is `r min(trip_data_v2$ride_length)` minutes, while the maximum duration reaches `r prettyNum(round(max(trip_data_v2$ride_length), 2), big.mark = ",")` minutes. The most frequent ride duration is `r round(find_mode(trip_data_v2$ride_length), 2)` minutes. The average duration of the rides is calculated to be `r round(mean(trip_data_v2$ride_length), 2)` minutes, and the median duration of rides stands at `r round(median(trip_data_v2$ride_length), 2)` minutes. 

 

```{r plot-distribution-duration-all}
# Define text details for the plot
label_01 <- labs(
...
annotation_01 <- annotate(
  geom = "text", 
  label = c("AVG = 16.61 min.", "MED = 10.52 min.", "MOD = 5.38 min."), 
...
# Create histogram of ride duration
ggplot(trip_data_v2, aes(x = ride_length)) + 
  geom_histogram(binwidth = 2, color = "black", fill = "darkgray") +
  ...
  geom_vline(
    aes(xintercept = median(ride_length)), 
    linetype = "dashed", 
    size = 0.5, 
    color = "#c76a27"
  ) + 
  geom_vline(
    aes(xintercept = find_mode(ride_length)), 
    linetype = "dashed", 
    size = 0.5, 
    color = "#c76a27"
  ) + 
  # Add text annotation for vertical average line
  annotation_01 + 
  # Format x-axis and the y-axis 
  xlim(0, 60) + 
  expand_limits(y = c(0, 1000000)) + 
  scale_y_continuous(labels = unit_format(unit = "k", scale = 1e-3)) + 
  # Add title, legend and color theme
  label_01 + 
...
# Create bar chart of average ride length grouped by user type
ggplot(stats_user, aes(x = member_casual, y = mean_ride_length, fill = member_casual)) + 
  geom_bar(stat = "identity", color = "black") +
  # Add text for average ride duration
  geom_text(
    aes(label = round(mean_ride_length, 2)), 
    position = position_dodge(width = 0.9), 
    vjust = 2
  ) + 
  # Add text annotations for value difference
  annotation_03_1 + annotation_03_2 +
  # Format x-axis and the y-axis 
...
# Create bar chart of ride count grouped by user type
ggplot(stats_user, aes(x = member_casual, y = ride_count, fill = member_casual)) + 
  geom_bar(stat = "identity", color = "black") + 
  # Add text for ride count
  geom_text(
  ...
  # Add text annotations for value difference
  annotation_04_1 + annotation_04_2 +
  # Format x-axis and the y-axis 
...
label_05 <- labs(
  ...
  title = "Casual riders tend to ride longer in spring", 
  subtitle = "Average ride duration for casual riders and annual members by month, 2022", 
    caption = "Source: Cyclistic (Divvy) dataset with 5,541,268 ride records"
  )

# Define rectangular annotation for key highlight
annotation_05 <- annotate(
  "rect", 
  xmin = 3, xmax = 5, 
  ymin = 0, ymax = 30,
  fill = "#2C7EB8",
...
# Create line chart of average ride length grouped by user type and month
ggplot(stats_user_month, aes(x = month, y = mean_ride_length, group = member_casual)) + 
  ...
  # Add text annotation for key highlight
  annotation_05 +
  # Format x-axis and the y-axis 
  expand_limits(y = c(0, 30)) + 
  # Add title, legend and color theme
  label_05 + labs(color = "User Type") + 
  plot_theme + legend_theme + theme(legend.position = c(0.9, 0.85)) + 
...
label_06 <- labs(
  x = "Month", 
  y = "Number of rides", 
  title = "Cyclistic bike usage peaks at spring and summer", 
  subtitle = "Total number of rides for casual riders and annual members by month, 2022", 
  caption = "Source: Cyclistic (Divvy) dataset with 5,541,268 ride records"
  )

# Define text annotation for key highlight
annotation_06 <- annotate(
  "rect", 
  xmin = 6, xmax = 8, 
  ymin = 0, ymax = 500000,
  fill = "darkgray", 
...
# Create line chart of ride count grouped by user type and month
...
label_07 <- labs(
    ...
    y = "Duration of ride (in minutes)",
    title = "Test",
    subtitle = "Average ride duration for casual riders and annual member by day of week, 2022",
    caption = "Source: Cyclistic (Divvy) dataset with 5,541,268 ride records"
  )

# Create bar chart of average ride length grouped by user type and day of week
ggplot(stats_user_day, aes(x = day_of_week, y = mean_ride_length, fill = member_casual)) +
...
label_08 <- labs(
  ...
  y = "Number of rides", 
  title = "Test", 
  subtitle = "Total number of rides for casual riders and annual members by day of week, 2022", 
  caption = "Source: Cyclistic (Divvy) dataset with 5,541,268 ride records"
)
...
label_09 <- labs(
    ...
    y = "Ride duration (in minutes)", 
  title = "Test", 
  subtitle = "Average ride duration for casual riders and annual members by hour, 2022", 
    caption = "Source: Cyclistic (Divvy) dataset with 5,541,268 ride records"
  )

# Create line chart of average ride length grouped by user type and month
ggplot(stats_user_hour, aes(x = hour, y = mean_ride_length, group = member_casual)) + 
  geom_line(aes(color = member_casual), size = 1) + 
  geom_point(aes(color = member_casual), size = 2) + 
  # Add text annotation for key highlight
  # annotation_09 +
...
label_10 <- labs(
  ...
  y = "Number of rides", 
  title = "Test", 
  subtitle = "Total number of rides for casual riders and annual members by hour, 2022", 
  caption = "Source: Cyclistic (Divvy) dataset with 5,541,268 ride records"
  )

# Define text annotation for peak line
annotation_10 <- annotate(
  geom = "text",
  label = c("8 AM", "5 PM"),
  x = c(8, 17),
  y = Inf,
...
ggplot(stats_user_hour, aes(x = hour, y = ride_count, group = member_casual)) + 
  ...
  # Add text annotation for peak line
  annotation_10 +
  # Format x-axis and the y-axis 
  expand_limits(y = c(0, 400000)) + 
  scale_y_continuous(labels = unit_format(unit = "k", scale = 1e-3)) + 
  # Add title, legend and color theme
  label_10 + labs(color = "User Type") + 
  plot_theme + legend_theme + theme(legend.position = c(0.9, 0.85)) + 
  scale_color_brewer(palette = "YlGnBu")
```

<!-- # ```{r plot-line-count-user-hour-day} -->
<!-- # trip_data_v2 %>%  -->
<!-- #   mutate(hour = as.numeric(format(started_at, "%H"))) %>%  -->
...
<!-- #     .groups = "drop" -->
<!-- #   ) %>%  -->
<!-- #   ggplot(aes(x = hour, y = ride_count, group = member_casual)) +  -->
<!-- #   geom_line(aes(color = member_casual), size = 1) +  -->
<!-- #   geom_point(aes(color = member_casual), size = 2) +  -->
<!-- #   geom_vline( -->
<!-- #     xintercept = c(8, 17),  -->
...
<!-- # ```{r plot-line-duration-user-hour-day} -->
<!-- # trip_data_v2 %>%  -->
<!-- #   mutate(hour = as.numeric(format(started_at, "%H"))) %>%  -->
<!-- #   group_by(member_casual, hour, day_of_week) %>%  -->
<!-- #   summarize( -->
...
<!-- #     .groups = "drop" -->
<!-- #   ) %>%  -->
<!-- #   ggplot(aes(x = hour, y = mean_ride_length, group = member_casual)) +  -->
<!-- #   geom_line(aes(color = member_casual), size = 1) +  -->
<!-- #   geom_point(aes(color = member_casual), size = 2) +  -->
<!-- #   geom_vline( -->
<!-- #     xintercept = c(8, 17),  -->
