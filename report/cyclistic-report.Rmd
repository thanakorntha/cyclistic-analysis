...
theme_nun <- function(base_size = 12) {
  theme_bw(base_size = base_size) %+replace%
    theme(
      # Figure
      plot.title = element_text(size = rel(1), face = "bold", margin = margin(0,0,10,0), hjust = 0),
      plot.subtitle = element_text(size = rel(0.85), margin = margin(0,0,5,0), hjust = 0),
      plot.caption = element_text(size = rel(0.70), face = "italic", margin = margin(5,0,5,0), hjust = 0),
      # Graph area
      panel.grid.minor = element_blank(),
      panel.grid.major.x = element_blank(),
      panel.border = element_blank(),
      # Axis
      axis.title = element_text(size = rel(0.70), face = "bold"),
      axis.text = element_text(size = rel(0.55), face = "bold"),
      axis.line = element_line(color = "black", arrow = arrow(length = unit(0.30, "lines"), type = "closed")),
      # Legend
      legend.title = element_blank(), # element_text(size = rel(0.85), face = "bold"),
...
## The Difference of Bike Usage between Casual Riders and Annual Members
...
plot_02 <- ggplot(stats_user, aes(x = member_casual, y = median_ride_length, fill = member_casual)) +
  ...
  labs(
    ...
    title = "Median Time Duration on Cyclistic Bikes"
  )
...
plot_03 <- ggplot(stats_user, aes(x = member_casual, y = ride_count, fill = member_casual)) +
  ...
  labs(
    x = "User Type",
    y = "Number of Rides",
    title = "Total Number of Cyclistic Rides"
  )
...
plot_03 +
  ...
  annotate(
    "segment",
    x = 1, xend = 1,
    y = (3271770 / 1.03), yend = (2269498 * 1.03),
    arrow = arrow(ends = "both", angle = 90, length = unit(0.25,"cm")),
    color = "red"
  ) +
...
# Create a bar chart
plot_04 <- ggplot(stats_user_day, aes(x = day_of_week, y = median_ride_length, fill = member_casual)) +
  geom_bar(stat = "identity", position = "dodge", color = "black") +
  labs(
    x = "Day of Week",
    y = "Time Duration (in minutes)",
    title = "Median Time Duration on Cyclistic Bikes by Day"
  )
...
plot_05 <- ggplot(stats_user_day, aes(x = day_of_week, y = ride_count, fill = member_casual)) +
  ...
  labs(
    x = "Day of Week",
    y = "Number of Rides", 
    title = "Total Number of Cyclistic Rides by Day"
  )
...
plot_06 <- ggplot(stats_user_hour, aes(x = hour, y = median_ride_length, group = member_casual)) +
  ...
  labs(
    x = "Hour",
    y = "Time Duration (in minutes)",
    title = "Median Time Duration on Cyclistic Bikes by Hour"
  )
# Add annotation
plot_06 +
  geom_hline(yintercept = median(trip_data_v2$ride_length), color = "darkgray", linetype = "dashed", size = 0.50) +
  annotate("text", label = "Median = 10.52", x = 23, y = median(trip_data_v2$ride_length), vjust = -1, hjust = 0.75, size = 2.50, color = "darkgray")
  # annotate(geom = "text", label = c("8 AM", "12 PM", "5 PM"), x = c(8, 12, 17), y = Inf, vjust = 1.50, hjust = -0.20, size = 2.50, color = "darkgray")
```
...
plot_07 <- ggplot(stats_user_hour, aes(x = hour, y = ride_count, group = member_casual)) +
  ...
  labs(
    x = "Hour",
    y = "Number of Rides", 
    title = "Total Number of Cyclistic Rides by Hour"
  )

# Add annotation
plot_07 +
  # geom_vline(xintercept = c(8, 12, 17), linetype = "dashed", size = 0.50, color = "darkgray") +
  # annotate(geom = "text", label = c("8 AM", "12 PM", "5 PM"), x = c(8, 12, 17), y = Inf, vjust = 1.50, hjust = -0.20, size = 2.50, color = "darkgray")
  # Annotation for casual's peak
  annotate("segment", x = 17, xend = 17, y = 120000, yend = 190000, arrow = arrow(type = "closed", length = unit(0.02, "npc")), color = "darkgray") +
  annotate("text", x = 17, y = 100000, size = 2.50, color = "gray40", label = "Casual riders' bike usage \n also peaks at 5 PM") +
  # Annotation for member's peak
  annotate("segment", x = 5, xend = 7.75, y = 375000, yend = 220000, arrow = arrow(type = "closed", length = unit(0.02, "npc")), color = "darkgray") +
  annotate("segment", x = 5, xend = 11.50, y = 375000, yend = 200000, arrow = arrow(type = "closed", length = unit(0.02, "npc")), color = "darkgray") +
  annotate("segment", x = 5, xend = 16.50, y = 375000, yend = 343000, arrow = arrow(type = "closed", length = unit(0.02, "npc")), color = "darkgray") +
  annotate("text", x = 5, y = 400000, size = 2.50, color = "gray40", label = "Annual members' bike usage \n peaks at 8 AM, 12 PM and 5 PM")
  # # Annotation for overall trend
  # annotate("segment", x = 5, xend = 17, y = 0, yend = 0, arrow = arrow(ends = "both", angle = 90, length = unit(0.10,"cm")), color = "darkgray") #+
  # annotate(
  #   "text", x = 6.50, y = 30000, size = 2.50, color = "gray50", 
  #   label = "The number of users gradually increases from February to a peak in July and August, \n followed by a decrease in the remaining months of the year"
  # )
```
...
plot_08 <- ggplot(stats_user_month, aes(x = month, y = median_ride_length, group = member_casual)) +
  geom_line(aes(color = member_casual), size = 1) +
  geom_point(aes(color = member_casual), size = 2) +
  # geom_line(aes(color = member_casual), stat = "smooth", linetype = "longdash", alpha = 0.20) +
  expand_limits(y = c(0, 20)) +
  labs(
    x = "Month", 
    y = "Time Duration (in minutes)", 
    title = "Median Time Duration on Cyclistic Bikes by Month"
  )

# Add annotation
plot_08 +
  # Median line
  geom_hline(yintercept = median(trip_data_v2$ride_length), color = "darkorange", linetype = "dashed", size = 0.50) +
  annotate("text", label = "Median = 10.52", x = 12, y = median(trip_data_v2$ride_length), vjust = -1, hjust = 0.75, size = 2.50, color = "darkorange") +
  # Annotation for casual's trend
  annotate("segment", x = 3, xend = 6, y = 17, yend = 17, arrow = arrow(ends = "both", angle = 90, length = unit(0.15,"cm")), color = "darkgray") +
  annotate("text", x = 4.50, y = 18.25, size = 2.50, color = "gray40", label = "Casuals took a longer time \n on bikes from March to May")
```
...
plot_09 <- ggplot(stats_user_month, aes(x = month, y = ride_count, group = member_casual)) +
  ...
  labs(
    x = "Month", 
    y = "Number of Rides", 
    title = "Total Number of Cyclistic Rides by Month",
  )
